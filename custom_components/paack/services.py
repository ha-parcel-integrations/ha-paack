"""Services for the Paack parcel tracker integration.

`paack.track_parcel` / `paack.untrack_parcel` let you add or remove a
...
_TRACK_SCHEMA = vol.Schema({vol.Required(CONF_TRACKING_CODE): cv.string, vol.Optional(CONF_POSTAL_CODE): cv.string})
_UNTRACK_SCHEMA = vol.Schema({vol.Required(CONF_TRACKING_CODE): cv.string})


def _resolve_entry(hass: HomeAssistant, postal_code: str | None = None):
    """Return the selected hub, requiring a postcode when ambiguous."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("Paack is not set up")
    if postal_code:
        target = normalize_postcode(postal_code)
        for entry in entries:
            if entry.options.get(CONF_POSTAL_CODE) == target:
                return entry
        raise ServiceValidationError("No Paack hub for this postal code")
    if len(entries) == 1:
        return entries[0]
    raise ServiceValidationError("Multiple Paack hubs are set up — pass postal_code")


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the Paack services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid Paack tracking code"
            )
        entry = _resolve_entry(hass, call.data.get(CONF_POSTAL_CODE))
        if not valid_postcode(entry.options.get(CONF_POSTAL_CODE, "")):
            raise ServiceValidationError("The Paack hub has no valid postal code")
    ...
    async def _untrack(call: ServiceCall) -> None:
        ...
        for entry in hass.config_entries.async_entries(DOMAIN):
            ...
            if len(kept) != len(current):
                hass.config_entries.async_update_entry(entry, options={**entry.options, CONF_PARCELS: kept})
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the Paack services (single-entry integration, so on unload)."""
