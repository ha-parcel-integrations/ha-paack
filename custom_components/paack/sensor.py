"""Sensor platform for the Paack parcel tracker integration."""
...
from . import PaackConfigEntry
from .const import DOMAIN, ParcelStatus
from .coordinator import PaackCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: PaackConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Paack sensor entities from a config entry."""
    ...
    for parcel in coordinator.data or []:
        entities.append(
            PaackParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(PaackNextDeliverySensor(coordinator, entry))
    entities.append(PaackDeliveredParcelsSensor(coordinator, entry))
    entities.append(PaackAwaitingPickupSensor(coordinator, entry))
    entities.append(PaackLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class PaackIncomingParcelsSensor(
    CoordinatorEntity[PaackCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                PaackParcelSensor(self.coordinator, self._entry, barcode)
...
class PaackParcelSensor(CoordinatorEntity[PaackCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Paack parcel."""
    ...
    def __init__(
        self, coordinator: PaackCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class PaackNextDeliverySensor(
    ...
    def __init__(
        self, coordinator: PaackCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class PaackDeliveredParcelsSensor(
    CoordinatorEntity[PaackCoordinator], SensorEntity
):
    """Recently delivered tracked Paack parcels."""
...
class PaackAwaitingPickupSensor(
    CoordinatorEntity[PaackCoordinator], SensorEntity
):
    """Parcels dropped at a Paack pickup point and ready to collect."""
...
class PaackLastUpdateSensor(
    CoordinatorEntity[PaackCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Paack was last polled successfully."""
