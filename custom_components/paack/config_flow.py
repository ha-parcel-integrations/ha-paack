"""Config flow for the Paack parcel tracker integration."""
...
_COUNTRY_SELECTOR = selector.SelectSelector(
    selector.SelectSelectorConfig(
        options=[country.lower() for country in SUPPORTED_COUNTRIES],
...
def _hub_schema(country: str) -> vol.Schema:
    """Build the hub setup schema, retaining the selected country on errors."""
    return vol.Schema(
        {
...
def normalize_postcode(value: str) -> str:
    """Return a postcode without spaces and upper-cased."""
    return re.sub(r"\s+", "", (value or "").upper())


def valid_postcode(value: str) -> bool:
    """Accept Paack's documented 3–10 alphanumeric postcode boundary."""
    return bool(re.fullmatch(r"[A-Z0-9-]{3,10}", value))
...
def _base_options(entry: ConfigEntry) -> dict[str, Any]:
    """Return hub fields that every options-flow save must preserve."""
    return {
        CONF_COUNTRY: entry.options.get(CONF_COUNTRY),
        CONF_POSTAL_CODE: entry.options.get(CONF_POSTAL_CODE),
    }
...
class PaackConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI-driven configuration flow for the Paack integration."""
    ...
    ) -> PaackOptionsFlowHandler:
        """Return the options flow handler."""
        return PaackOptionsFlowHandler()

    async def async_step_user(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Create a Paack country-and-postcode hub without a network call."""
        errors: dict[str, str] = {}
        country = "ES"
...
class PaackOptionsFlowHandler(OptionsFlow):
        ...
        if user_input is not None:
            ...
            if any(not valid_tracking_code(code) for code in codes):
                ...
                return self.async_create_entry(
                    title="",
                    data={
                        **_base_options(self.config_entry),
