"""Calendar platform for the Paack parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Paack deliveries calendar from a config entry."""
    async_add_entities([PaackDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class PaackDeliveriesCalendar(CoordinatorEntity[PaackCoordinator], CalendarEntity):
    """A read-only calendar of expected Paack deliveries.
    ...
    def __init__(self, coordinator: PaackCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Paack parcel")
            description_parts = [
