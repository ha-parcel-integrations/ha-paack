"""Paack parcel tracker custom component for Home Assistant."""
...
from .api import PaackApiClient
...
@dataclass
class PaackData:
    """Runtime data attached to the Paack config entry."""

    client: PaackApiClient
    coordinator: PaackCoordinator


type PaackConfigEntry = ConfigEntry[PaackData]


async def async_setup_entry(hass: HomeAssistant, entry: PaackConfigEntry) -> bool:
    """Set up Paack from a config entry."""
    # No auth: Paack tracking is public, so the HA-managed session is fine.
    client = PaackApiClient(async_get_clientsession(hass))
    coordinator = PaackCoordinator(hass, client, entry)
    ...
    entry.runtime_data = PaackData(client=client, coordinator=coordinator)
...
async def _async_options_updated(
    hass: HomeAssistant, entry: PaackConfigEntry
) -> None:
    """Apply changed options by refreshing the coordinator."""
    await entry.runtime_data.coordinator.async_request_refresh()


async def async_unload_entry(hass: HomeAssistant, entry: PaackConfigEntry) -> bool:
    """Unload the Paack config entry."""
