"""Coordinator for the Paack parcel tracker integration.
...
from .api import PaackApiClient, PaackApiError
from .const import (
...
class PaackCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        results = await asyncio.gather(
            *(
                self._client.async_get_parcel(code, postal_code)
                if (postal_code := self.config_entry.options.get(CONF_POSTAL_CODE))
                else self._client.async_get_parcel(code)
        ...
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (PaackApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if (
                    isinstance(result, PaackApiError)
                    and result.status_code == 429
                ):
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("Paack fetch failed for %s: %s", code, result)
        ...
        if saw_429:
            ...
            raise UpdateFailed(
                "Paack rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("Paack unreachable for all tracked parcels")
