...
from custom_components.paack.parcels import (
...
def test_map_rescheduled_appointment_as_registered():
    assert map_parcel_status("appointmentChangedToNextAvailableDay") == ParcelStatus.REGISTERED
...
def test_build_history_caps_to_max_events():
    events = [
        event("scannedAtOrigin", f"2026-04-{day:02d}T10:00:00Z", "moved")
...
def test_build_history_handles_missing_and_malformed():
    ...
    assert build_history([{"statusCode": "scannedAtOrigin"}]) == []  # no timestamp
...
def test_build_history_keeps_unparseable_timestamp_last():
    history = build_history(
        [
            event("manifested", "2026-04-24T10:00:00Z", "fine"),
            event("scannedAtOrigin", "not-a-date", "odd"),
        ]
    )
...
def test_build_history_falls_back_to_status_code_without_text():
    history = build_history([event("scannedAtOrigin", "2026-04-24T10:00:00Z", "")])
    assert history[0]["raw_status"] == "scannedAtOrigin"
...
def test_normalize_delivered_parcel():
    ...
    assert parcel["url"] == "https://paack.co/shipments/tracking/order?tracking_number=EXAMPLE123456"
...
def test_normalize_loader_delivery_window():
    raw = {
        "orderTrackData": {
            "external_id": "SYNTHETIC123",
            "expected_delivery_ts": {
                "start": "2026-04-17T07:00:00.000Z",
                "end": "2026-04-17T20:00:00.000Z",
            },
        },
        "activeEvent": {"label": "inDelivery"},
    }
    parcel = normalize_parcel(raw)
    assert parcel["planned_from"] == "2026-04-17T07:00:00.000Z"
    assert parcel["planned_to"] == "2026-04-17T20:00:00.000Z"
...
def test_normalize_falls_back_to_status_code_without_text():
    raw = active_sample()
    raw["statusText"] = None
    assert normalize_parcel(raw)["raw_status"] == "inDelivery"
