"""Tests for Paack setup and unload."""
...
async def test_setup_and_unload(hass):
    ...
    incoming = hass.states.get("sensor.paack_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.paack.api.PaackApiClient.async_get_parcel",
        new=AsyncMock(side_effect=PaackApiError("Paack unreachable")),
    ):
...
async def test_per_parcel_sensor_spawn_and_remove(hass):
    ...
    with patch("custom_components.paack.api.PaackApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
