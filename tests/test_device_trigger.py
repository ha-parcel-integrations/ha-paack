"""Tests for Paack device triggers."""
from unittest.mock import AsyncMock, patch

from homeassistant.const import CONF_DEVICE_ID, CONF_TYPE

from custom_components.paack.const import DOMAIN
from custom_components.paack.device_trigger import (
    TRIGGER_EVENTS,
...
async def test_attach_trigger_delegates_to_event_trigger(hass):
    def callback():
        return None
    with patch(
        "custom_components.paack.device_trigger.event_trigger.async_attach_trigger",
        new=AsyncMock(return_value=callback),
    ) as attach:
    ...
    assert attach.call_args.kwargs["platform_type"] == "device"
