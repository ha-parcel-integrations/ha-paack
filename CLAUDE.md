...
Home Assistant custom integration for **Paack** parcel tracking.
...
private `carrier-research/paack/api/` and are **never** copied here.
...
- **If this carrier can reach `ParcelStatus.AT_PICKUP_POINT` from a real raw
  ...
  `ha-inpost` are reference implementations; Paack maps its documented PUDO
  states and exposes the awaiting-pickup sensor.

## Carrier-specific notes

Paack is account-less and postcode-hub based: an entry is one normalised
country/postcode pair and can hold many codes. Setup is offline; options and
services apply live. The coordinator makes one bounded request per active code,
skips delivered codes, honours 429 backoff and retains only parsed loader data
in runtime `raw`, never page HTML. Delivery and pickup estimates are optional;
weight and dimensions are intentionally `None`. Diagnostics redact every
identifier, address, contact and PUDO field. API mechanics remain private in
`carrier-research/paack/api/`.
...
Home Assistant (`PaackAuthError` from any per-parcel fetch raises
...
refresh. `paack/coordinator.py` is the canonical implementation
...
```
python -m pytest tests/ --cov=custom_components.paack
```
