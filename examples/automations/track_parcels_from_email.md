# Automatic parcel tracking from e-mail (IMAP → Paack)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Paack tracking code, and register it with `paack.track_parcel` — fully automatic, no extra custom component required.

Paack is a **code-based, postcode-gated** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `paack.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ paack.track_parcel
                                     │
...
- This integration, with the `paack.track_parcel` action available.
- An `input_text.paack_delivery_postcode` helper containing the delivery postcode for the hub this automation owns. This is required even with one hub, so adding another hub later cannot make the automation ambiguous.
...
Paack codes have no stable public prefix. The integration accepts any non-empty code, but a broad e-mail regex would also capture order numbers. Restrict this automation to a known retailer sender and adapt its pattern to that retailer's confirmation mail; the AI fallback can handle layouts that do not have a safe deterministic pattern.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@paackshipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: EXAMPLE123456","initial":true,"folder":"INBOX","username":"test"}'
```

Then `paack.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
